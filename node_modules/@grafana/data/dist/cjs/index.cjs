...
function getFieldTypeForReducer(id, fallback) {
  return id === "count" /* count */ ? FieldType.number : id === "allIsNull" /* allIsNull */ || id === "allIsZero" /* allIsZero */ ? FieldType.boolean : fallback;
}
...
function reduceField(options) {
  ...
  if ((_a = field.state) == null ? undefined : _a.calcs) {
    const missing = [];
...
function fieldNameFallback(fields) {
  ...
  const useMatcherFallback = (_c = (_b = (_a = window == null ? undefined : window.grafanaBootData) == null ? undefined : _a.settings) == null ? undefined : _b.featureToggles) == null ? undefined : _c.dataplaneFrontendFallback;
  if (useMatcherFallback) {
    if (fields.has(TIME_SERIES_VALUE_FIELD_NAME)) {
      fallback = (field, frame) => {
        ...
        ((_a2 = field.labels) == null ? undefined : _a2.__name__) === field.name;
      };
    } else if (fields.has("Time") || fields.has("time")) {
      fallback = (field, frame) => {
        var _a2;
        return ((_a2 = frame.meta) == null ? undefined : _a2.typeVersion) == null && field.type === FieldType.time;
      };
    }
  }
  return fallback;
...
const regexpOrMultipleNamesMatcher = {
  ...
  get: (options) => {
    var _a;
    const regexpMatcher = regexpFieldNameMatcher.get((options == null ? undefined : options.pattern) || "");
    const namesMatcher = multipleFieldNamesMatcher.get({
      mode: "include" /* include */,
      names: (_a = options == null ? undefined : options.names) != null ? _a : []
    });
    return (field, frame, allFrames) => {
  ...
  getOptionsDisplayText: (options) => {
    var _a, _b, _c;
    const pattern = (_a = options == null ? undefined : options.pattern) != null ? _a : "";
    const names = (_c = (_b = options == null ? undefined : options.names) == null ? undefined : _b.join(",")) != null ? _c : "";
...
function getFieldDisplayName(field, frame, allFrames) {
  var _a, _b;
  const existingTitle = (_a = field.state) == null ? undefined : _a.displayName;
  const multipleFrames = Boolean(allFrames && allFrames.length > 1);
  if (existingTitle && multipleFrames === ((_b = field.state) == null ? undefined : _b.multipleFrames)) {
...
function calculateFieldDisplayName(field, frame, allFrames) {
  var _a, _b, _c, _d, _e;
  const hasConfigTitle = ((_a = field.config) == null ? undefined : _a.displayName) && ((_b = field.config) == null ? undefined : _b.displayName.length);
  const isComparisonSeries = Boolean((_d = (_c = frame == null ? undefined : frame.meta) == null ? undefined : _c.timeCompare) == null ? undefined : _d.isTimeShiftQuery);
  ...
  if (frame && ((_e = field.config) == null ? undefined : _e.displayNameFromDS)) {
  ...
  if (frameNamesDiffer && (frame == null ? undefined : frame.name)) {
...
function arrayToDataFrame(source, names) {
  ...
  if (!(source == null ? undefined : source.length)) {
    return df;
  }
  if (names) {
...
function dataFrameFromJSON(dto) {
  ...
  const fields = schema.fields.map((f, index) => {
    ...
    let entities = (_a = data == null ? undefined : data.entities) == null ? undefined : _a[index];
    if (entities) {
      decodeFieldValueEntities(entities, buffer);
    }
    let enums = (_b = data == null ? undefined : data.enums) == null ? undefined : _b[index];
    if (enums) {
      decodeFieldValueEnums(enums, buffer);
      type = FieldType.string;
    }
    const nanos = (_c = data == null ? undefined : data.nanos) == null ? undefined : _c[index];
...
function convertTableToDataFrame(table) {
  const fields = table.columns.map((c) => {
    ...
    const values = [];
    return {
      name: (text == null ? undefined : text.length) ? text : c,
...
function toDataFrame(data) {
  var _a, _b;
  if ("fields" in data) {
    if ("length" in data && ((_b = (_a = data.fields[0]) == null ? undefined : _a.values) == null ? undefined : _b.get)) {
      return data;
    }
...
function joinDataFrames(options) {
  var _a, _b, _c, _d, _e;
  if (!((_a = options.frames) == null ? undefined : _a.length)) {
    return;
  }
  const nullMode = (_b = options.nullMode) != null ? _b : (field) => {
    var _a2;
    let spanNulls = (_a2 = field.config.custom) == null ? undefined : _a2.spanNulls;
  ...
  for (let frameIndex = 0; frameIndex < options.frames.length; frameIndex++) {
    ...
    if (!frame || !((_c = frame.fields) == null ? undefined : _c.length)) {
      continue;
    }
  ...
  constructor(options) {
    ...
    this.serialize = (fieldPredicate, optionsOverride, trimValues) => {
      ...
      const numberOfItemsToRemove = getNumberOfItemsToRemove(
        ...
        typeof (trimValues == null ? undefined : trimValues.maxLength) === "number" ? Math.min(trimValues.maxLength, options.maxLength) : options.maxLength,
        this.timeFieldIndex,
        options.maxDelta
      );
      dataFrameDTO.fields = dataFrameDTO.fields.map((f) => {
        ...
        return {
          ...f,
          values: (_a2 = f.values) == null ? undefined : _a2.slice(numberOfItemsToRemove)
        };
      });
      const length = (_c = (_b = (_a = dataFrameDTO.fields[0]) == null ? undefined : _a.values) == null ? undefined : _b.length) != null ? _c : 0;
      return {
        ...dataFrameDTO,
    ...
    this.pushNewValues = (values) => {
      var _a, _b;
      if (!(values == null ? undefined : values.length)) {
        return;
      }
      ...
      const newLength = (_b = (_a = this.fields) == null ? undefined : _a[0]) == null ? undefined : _b.values.length;
    ...
    this.getMatchingFieldIndexes = (fieldPredicate) => this.fields.map((f, index) => fieldPredicate(f) ? index : undefined).filter((val) => val !== undefined);
      ...
      if (hasSameStructure(this.schemaFields, niceSchemaFields)) {
        ...
        this.fields = niceSchemaFields.map((f) => {
          ...
          return {
            ...
            values: isWide ? (_d = (_c = this.fields.find((of) => of.name === f.name && f.type === of.type)) == null ? undefined : _c.values) != null ? _d : Array(this.length).fill(undefined) : []
          };
        });
      }
...
function getStreamingFrameOptions(opts) {
  var _a, _b, _c;
  return {
    maxLength: (_a = opts == null ? undefined : opts.maxLength) != null ? _a : 1e3,
    maxDelta: (_b = opts == null ? undefined : opts.maxDelta) != null ? _b : Infinity,
    action: (_c = opts == null ? undefined : opts.action) != null ? _c : "append" /* Append */,
    displayNameFormat: opts == null ? undefined : opts.displayNameFormat
  };
}
...
function getNumberOfItemsToRemove(data, maxLength = Infinity, deltaIdx = 0, maxDelta = Infinity) {
  var _a;
  if (!((_a = data[0]) == null ? undefined : _a.length)) {
    return 0;
  }
...
function hasSameStructure(a, b) {
  if ((a == null ? undefined : a.length) !== b.length) {
    return false;
  }
...
class FieldCache {
  ...
  getFirstFieldOfType(type, includeHidden = false) {
    const fields = this.fieldByType[type];
    const firstField = fields == null ? undefined : fields.find((field) => {
      var _a;
      return includeHidden || !((_a = field.config.custom) == null ? undefined : _a.hidden);
    });
    return firstField;
  }
  hasFieldNamed(name) {
...
class MutableDataFrame extends FunctionalVector {
  ...
  parseValue(field, v) {
    var _a;
    let p = (_a = this.parsers) == null ? undefined : _a.get(field);
    if (!p) {
...
function getRowUniqueId(dataFrame, rowIndex) {
  var _a;
  if (((_a = dataFrame.meta) == null ? undefined : _a.uniqueRowIdFields) === undefined) {
    return undefined;
  }
...
function compareDataFrameStructures(a, b, skipConfig) {
  ...
  if (((_a = a == null ? undefined : a.fields) == null ? undefined : _a.length) !== ((_b = b == null ? undefined : b.fields) == null ? undefined : _b.length)) {
    return false;
  }
  if (a.name !== b.name) {
    return false;
...
function compareArrayValues(a, b, cmp) {
  ...
  if ((a == null ? undefined : a.length) !== (b == null ? undefined : b.length)) {
    return false;
  }
  for (let i = 0; i < a.length; i++) {
    if (!cmp(a[i], b[i])) {
...
const XSSWL = Object.keys(xss__namespace.whiteList).reduce((acc, element) => {
  var _a;
  acc[element] = (_a = xss__namespace.whiteList[element]) == null ? undefined : _a.concat(["class", "style"]);
  return acc;
}, {});
  ...
  getCalculator(field, theme) {
    ...
    if (this.isByValue) {
      ...
      return (_, _percent, _threshold) => {
        var _a, _b;
        return colors[Math.abs(stringHash__default.default((_b = (_a = field.state) == null ? undefined : _a.displayName) != null ? _b : field.name)) % colors.length];
      };
    } else {
      return (_, _percent, _threshold) => {
        var _a, _b;
        const seriesIndex = (_b = (_a = field.state) == null ? undefined : _a.seriesIndex) != null ? _b : 0;
...
function getFieldColorModeForField(field) {
  var _a, _b;
  return fieldColorModeRegistry.get((_b = (_a = field.config.color) == null ? undefined : _a.mode) != null ? _b : FieldColorModeId.Thresholds);
}
...
function getFieldSeriesColor(field, theme) {
  ...
  const stat = (_b = (_a = field.config.color) == null ? undefined : _a.seriesBy) != null ? _b : "last";
...
function getFixedColor(field, theme) {
  return () => {
    var _a, _b;
    return theme.visualization.getColorByName((_b = (_a = field.config.color) == null ? undefined : _a.fixedColor) != null ? _b : FALLBACK_COLOR);
  };
}
function getShadedColor(field, theme) {
  return () => {
    ...
    const baseColorString = theme.visualization.getColorByName(
      (_b = (_a = field.config.color) == null ? undefined : _a.fixedColor) != null ? _b : FALLBACK_COLOR
    );
    const colors = [
      baseColorString
    ...
    const seriesIndex = (_d = (_c = field.state) == null ? undefined : _c.seriesIndex) != null ? _d : 0;
    return colors[seriesIndex % colors.length];
  };
}
...
function mapInternalLinkToExplore(options) {
  ...
  const query = typeof ((_a = link.internal) == null ? undefined : _a.query) === "function" ? link.internal.query({ replaceVariables, scopedVars }) : internalLink.query;
  const interpolatedQuery = interpolateObject(query, scopedVars, replaceVariables);
  const interpolatedPanelsState = interpolateObject((_b = link.internal) == null ? undefined : _b.panelsState, scopedVars, replaceVariables);
  const interpolatedCorrelationData = interpolateObject((_c = link.meta) == null ? undefined : _c.correlationData, scopedVars, replaceVariables);
  ...
  return {
    ...
    target: (link == null ? undefined : link.targetBlank) ? "_blank" : "_self",
    origin: field
  };
}
  ...
  return locationUtil.assureBaseUrl(
    `/explore?left=${encodeURIComponent(
      serializeStateToUrlParam({
        ...
        ...(range == null ? undefined : range.raw) ? { range: toURLRange(range.raw) } : {},
...
const getTimeZone = (options) => {
  var _a;
  if ((options == null ? undefined : options.timeZone) && !lodash.isEmpty(options.timeZone)) {
    ...
    for (let i = 0; i < panel.valueMaps.length; i++) {
      ...
      mappings.push(
        upgradeOldAngularValueMapping(
          ...
          ((_b = (_a = panel.fieldConfig) == null ? undefined : _a.defaults) == null ? undefined : _b.thresholds) || migratedThresholds
        )
      );
    }
    ...
    for (let i = 0; i < panel.rangeMaps.length; i++) {
      ...
      mappings.push(
        upgradeOldAngularValueMapping(
          ...
          ((_d = (_c = panel.fieldConfig) == null ? undefined : _c.defaults) == null ? undefined : _d.thresholds) || migratedThresholds
        )
      );
    }
  }
...
function sci(value, decimals) {
  ...
  return { text: value.toExponential(decimals != null ? decimals : undefined) };
}
...
const getFormat = (options) => {
  var _a, _b;
  if (options == null ? undefined : options.defaultWithMS) {
    return (_a = options == null ? undefined : options.format) != null ? _a : systemDateFormats.fullDateMS;
  }
  return (_b = options == null ? undefined : options.format) != null ? _b : systemDateFormats.fullDate;
};
...
function currency(symbol, asSuffix) {
  ...
  return (value, decimals, scaledDecimals) => {
    ...
    if (isNegative) {
      scaled.prefix = `-${((_a = scaled.prefix) == null ? undefined : _a.length) ? scaled.prefix : ""}`;
    }
    return scaled;
  };
}
...
function locale(value, decimals) {
  ...
  return {
    text: value.toLocaleString(undefined, { maximumFractionDigits: decimals != null ? decimals : undefined })
  };
}
...
function getDisplayProcessor(options) {
  ...
  const hasCurrencyUnit = unit == null ? undefined : unit.startsWith("currency");
...
function setFieldConfigDefaults(config, defaults, context) {
  ...
  const defaultBaseStep = ((_a = defaults == null ? undefined : defaults.thresholds) == null ? undefined : _a.mode) === schema.ThresholdsMode.Absolute && ((_b = defaults.thresholds) == null ? undefined : _b.steps.find((step) => step.value === -Infinity));
  if (((_c = config.thresholds) == null ? undefined : _c.mode) === schema.ThresholdsMode.Absolute && !config.thresholds.steps.some((step) => step.value === -Infinity) && defaultBaseStep) {
...
function useFieldOverrides(plugin, fieldConfig, data, timeZone, theme, replace, dataLinkPostProcessor) {
  const fieldConfigRegistry = plugin == null ? undefined : plugin.fieldConfigRegistry;
  const structureRev = react.useRef(0);
  const prevSeries = usePrevious__default.default(data == null ? undefined : data.series);
  return react.useMemo(() => {
    if (!fieldConfigRegistry || !fieldConfig || !data) {
      return;
    }
    const series = data == null ? undefined : data.series;
...
function getFieldDataContextClone(frame, field, fieldScopedVars) {
  if (fieldScopedVars == null ? undefined : fieldScopedVars.__dataContext) {
    return {
      value: {
...
const getFieldDisplayValues = (options) => {
  ...
  for (let s = 0; s < data.length && !hitLimit; s++) {
    ...
    for (let i = 0; i < dataFrame.fields.length && !hitLimit; i++) {
      ...
      if ((_b = field.state) == null ? undefined : _b.range) {
        config = {
          ...config,
          ...(_c = field.state) == null ? undefined : _c.range
        };
      }
      ...
      if (reduceOptions.values) {
        ...
        for (const calc of calcs) {
          const scopedVars = (_g = (_f = field.state) == null ? undefined : _f.scopedVars) != null ? _g : {};
          ...
          displayValue.percentChange = options.percentChange ? reduceField({ field, reducers: [ReducerID.diffperc] }).diffperc : undefined;
...
function hasLinks(field) {
  var _a, _b;
  return ((_b = (_a = field.config) == null ? undefined : _a.links) == null ? undefined : _b.length) ? field.config.links.length > 0 : false;
}
...
function getFieldScopedVarsWithDataContexAndRowIndex(field, rowIndex) {
  ...
  if ((_b = (_a = field.state) == null ? undefined : _a.scopedVars) == null ? undefined : _b.__dataContext) {
    return {
      ...(_c = field.state) == null ? undefined : _c.scopedVars,
      __dataContext: {
        value: {
          ...(_e = (_d = field.state) == null ? undefined : _d.scopedVars) == null ? undefined : _e.__dataContext.value,
          rowIndex
        }
      }
    };
  }
  return (_f = field.state) == null ? undefined : _f.scopedVars;
}
...
const displayNameOverrideProcessor = (value, context, settings) => {
  var _a, _b;
  (_b = (_a = context.field) == null ? undefined : _a.state) == null ? true : delete _b.displayName;
...
function isDataSourceRef(ref) {
  return typeof ref === "object" && typeof (ref == null ? undefined : ref.uid) === "string";
}
...
function sortValues(sort) {
  return (a, b) => {
    ...
    if (a == null || typeof a === "string" && (a == null ? undefined : a.trim()) === "") {
      return 1;
    }
...
class Store {
  ...
  getBool(key, def) {
    if (def !== undefined && !this.exists(key)) {
      return def;
    }
...
function fieldToStringField(field, dateFormat, parseOptions, joinWith) {
  ...
  switch (field.type) {
    ...
    case FieldType.other:
      values = values.map((v) => {
        if ((joinWith == null ? undefined : joinWith.length) && Array.isArray(v)) {
...
const applyFormatTime = ({ timeField, outputFormat, timezone }, data, ctx) => {
  ...
  outputFormat = (_a = ctx == null ? undefined : ctx.interpolate(outputFormat)) != null ? _a : outputFormat;
  return data.map((frame) => ({
    ...frame,
    fields: frame.fields.map((field) => {
      var _a2;
      if (((_a2 = field.state) == null ? undefined : _a2.displayName) === timeField) {
...
function createSubframe(fields, frameLength, options) {
  const showHeaders = options.showSubframeHeaders === undefined ? SHOW_NESTED_HEADERS_DEFAULT : options.showSubframeHeaders;
  return {
...
function groupToSubframes(valuesByGroupKey, options) {
  ...
  for (const [, value] of valuesByGroupKey) {
    ...
    for (const [fieldName, field] of Object.entries(value)) {
      ...
      } else if (fieldOpts.aggregations === undefined || fieldOpts.operation === "aggregate" /* aggregate */ && fieldOpts.aggregations.length === 0 || fieldOpts.operation === null || fieldOpts.operation === undefined) {
        nestedFields.push(field);
      }
    }
...
const supportDataplaneFallback = (_c = (_b = (_a$1 = window == null ? undefined : window.grafanaBootData) == null ? undefined : _a$1.settings) == null ? undefined : _b.featureToggles) == null ? undefined : _c.dataplaneFrontendFallback;
...
const histogramTransformer = {
  ...
  transformer: (options, ctx) => (data) => {
    ...
    let bucketSize, bucketOffset = undefined;
...
function getHistogramFields(frame) {
  var _a, _b;
  if (((_a = frame.meta) == null ? undefined : _a.type) === DataFrameType.HeatmapCells) {
    ...
    return fields;
  } else if (((_b = frame.meta) == null ? undefined : _b.type) === DataFrameType.HeatmapRows) {
    let minVals = [];
    let maxVals = [];
    let countVals = [];
    let minVal = "0";
  ...
  if (!xMin && xMax && (xMax == null ? undefined : xMax.values.length) > 1) {
...
function buildHistogram(frames, options, theme) {
  var _a, _b, _c;
  let bucketSize = options == null ? undefined : options.bucketSize;
  let bucketCount = (_a = options == null ? undefined : options.bucketCount) != null ? _a : DEFAULT_BUCKET_COUNT;
  let bucketOffset = (_b = options == null ? undefined : options.bucketOffset) != null ? _b : 0;
  ...
  if (options == null ? undefined : options.combine) {
...
function histogram(vals, getBucket, filterOut, sort) {
  ...
  filterOut.forEach((v) => hist.delete(v));
  let bins = [...hist.values()];
  bins.sort((a, b) => sort(a.value, b.value));
...
const labelsToFieldsTransformer = {
  ...
  transformer: (options) => (data) => {
    ...
    const result = [];
    const keepLabels = ((_a = options.keepLabels) == null ? undefined : _a.length) ? new Set(options.keepLabels) : undefined;
    for (const frame of data) {
      const newFields = [];
      const uniqueLabels = {};
      for (const field of frame.fields) {
...
const reduceTransformer = {
  ...
  operator: (options) => (source) => source.pipe(
    operators.map((data) => {
      var _a;
      if (!((_a = options == null ? undefined : options.reducers) == null ? undefined : _a.length)) {
        return data;
      }
...
function mergeResults(data) {
  if (!(data == null ? undefined : data.length)) {
    return undefined;
  }
  ...
  switch (zone) {
    ...
    case "" /* default */: {
      ...
      return {
        ...
        offsetInMins: 0,
        ...info,
        ianaName: (_b = info == null ? undefined : info.ianaName) != null ? _b : "",
        name: "Default",
        zone
      };
    }
    case "browser" /* localBrowserTime */: {
      ...
      return {
        ...
        ...info,
        name: "Browser Time",
        ianaName: (_d = info == null ? undefined : info.ianaName) != null ? _d : "",
        zone
      };
    }
    default:
...
function parseDuration(durationString) {
  return durationString.split(" ").reduce((acc, value) => {
    ...
    const rawLength = match == null ? undefined : match[1];
    const unit = match == null ? undefined : match[2];
    if (!(rawLength && unit)) {
      return acc;
    }
    const mapping = Object.entries(durationMap).find(([_, abbreviations]) => abbreviations == null ? undefined : abbreviations.includes(match[2]));
...
function isValidDuration(durationString) {
  ...
  for (const value of durationString.trim().split(" ")) {
    ...
    const key = (_a = Object.entries(durationMap).find(([_, abbreviations]) => abbreviations == null ? undefined : abbreviations.includes(match[2]))) == null ? undefined : _a[0];
    if (!key) {
      return false;
    }
  }
...
function applyFieldConfigDefaults(existingFieldConfig, plugin) {
  ...
  const result = {
    ...
    overrides: (_a = existingFieldConfig == null ? undefined : existingFieldConfig.overrides) != null ? _a : []
  };
...
function adaptFieldColorMode(plugin, fieldConfig, isAfterPluginChange) {
  ...
  if (color && color.settings) {
    ...
    const mode = fieldColorModeRegistry.getIfExists((_a = fieldConfig.defaults.color) == null ? undefined : _a.mode);
    ...
    if (colorSettings.byValueSupport && colorSettings.preferThresholdsMode && (mode == null ? undefined : mode.id) !== FieldColorModeId.Fixed) {
      if (!mode || !mode.isByValue) {
        fieldConfig.defaults.color = { mode: FieldColorModeId.Thresholds };
        return fieldConfig;
      }
    }
    if (colorSettings.bySeriesSupport && (mode == null ? undefined : mode.isByValue)) {
      fieldConfig.defaults.color = { mode: FieldColorModeId.PaletteClassic };
      return fieldConfig;
    }
  }
...
function parseLiveChannelAddress(id) {
  if (id == null ? undefined : id.length) {
...
function isValidLiveChannelAddress(addr) {
  return !!((addr == null ? undefined : addr.path) && addr.namespace && addr.scope);
}
...
class VisualizationSuggestionsBuilder {
  ...
  computeDataSummary() {
    var _a, _b;
    const frames = ((_a = this.data) == null ? undefined : _a.series) || [];
    ...
    for (const frame of frames) {
      ...
      if ((_b = frame.meta) == null ? undefined : _b.preferredVisualisationType) {
...
class ArrayVector extends Array {
  ...
  set buffer(values) {
    this.length = 0;
    const len = values == null ? undefined : values.length;
    if (len) {
      let chonkSize = 65e3;
...
function userHasPermission(action, user) {
  var _a;
  return !!((_a = user.permissions) == null ? undefined : _a[action]);
}
function userHasPermissionInMetadata(action, object) {
  var _a;
  return !!((_a = object.accessControl) == null ? undefined : _a[action]);
}
